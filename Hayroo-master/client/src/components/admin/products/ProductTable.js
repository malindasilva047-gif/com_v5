// C:\lakmal_code\com_v5\com_v5\Hayroo-master\client\src\components\admin\products\ProductTable.js
    ...
    <Fragment>
      <div className="col-span-1 overflow-auto bg-white shadow-lg p-4">
        ...
        <div className="text-sm text-gray-600 mt-2">
          Total {products ? products.length : 0} product found
        </div>
      </div>
    </Fragment>
  );
...
const ProductTable = ({ product, deleteProduct, editProduct }) => {
  // Image URL safe lookup logic
  const getImageUrl = () => {
    if (product.pImages && product.pImages.length > 0) {
      const img = product.pImages[0];
      return img.startsWith("http") ? img : `${apiURL}/uploads/products/${img}`;
    }
    ...
    <Fragment>
      <tr>
        <td className="p-2 text-left">
          {product.pName && product.pName.length > 20
            ? product.pName.substring(0, 20) + "..."
            : product.pName}
        </td>
        <td className="p-2 text-left">
          {product.pDescription && product.pDescription.length > 20
            ? product.pDescription.substring(0, 20) + "..."
            : product.pDescription || "N/A"}
        </td>
        ...
        <td className="p-2 text-center">
          {product.pStatus === "Active" ? (
            <span className="bg-green-200 text-green-800 rounded-full text-center text-xs px-2 py-1 font-semibold">
              {product.pStatus}
            </span>
          ) : (
            <span className="bg-red-200 text-red-800 rounded-full text-center text-xs px-2 py-1 font-semibold">
        ...
        {/* Fixed Crash: Safety guard applied for pCategory */}
        <td className="p-2 text-center">
        ...
        <td className="p-2">
          <div className="flex items-center justify-center space-x-1">
            <span
              ...
              className="cursor-pointer hover:bg-gray-100 rounded-lg p-1.5 transition-colors"
              title="Edit Product"
            >
              <svg
